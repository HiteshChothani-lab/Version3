...
namespace UserManagement.Entity
{
    ...
    public class StoreUserEntity : INotifyPropertyChanged
    {
        ...
        public string Column1TextDisplayOnTop
        {
            get => $"{this.Note}".Trim();
        }

        public string Column1TextDisplayOnBottom
        {
            get => $"{this.BtnAB}".Trim();
        }
        ...
        public string Column2ExpressTimeDisplay
        {
            get
            {
                if (string.IsNullOrWhiteSpace(this.ExpressTime))
                    return string.Empty;
                else
                {
                    try
                    {
                        DateTime dt = DateTime.ParseExact(this.ExpressTime, "yyyy-MM-dd HH:mm:ss", null);
